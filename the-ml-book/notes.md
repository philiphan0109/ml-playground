...
Recall that **word embeddings** are feature vectors that represent words. Where do these word embeddings come from? They are learned from data. There are many algorithms to learn word embeddings. We consider one, **word2vec**, and only one version of word2vec called **skip-gram**. 

In word embedding learning, the goal is to build amodel which we can use to convert a one-hot encoding of a word into a word embedding. Let's say our dictionary contains 10,000 words. The one-hot vector for each word is a 10,000-dimentional vector of all zeroes except for one dimension that contains a 1. 

Consider the sentence: "I almost finished reading the book on machine learning." If we remove the word "book". If I were to ask you what word was missing, you would probably guess, "article," "book," or "paper." That's how the context words let you predict the word they surround. It's also how the machine can learn that "book" "paper" and "article" have a similar meaning: they share similar contexts in multiple texts.

A word can also predict the context that surrounds it too. The piece "finished reading the ~ on machine learning" is called the **skip-gram** with window size 7. A skip-gram can be denoted like this: $[x_{-3}, x_{-2}, x_{-1}, x, x_{+1}, x_{+2}, x_{+3}]$. Th neural network has to learn to predict the context words of the skip-gram given the central word.

It's easy to see how this is **self-supervised**: the labeled examples get extracted from the unlabeled data. The activation function used in the output layer is softmax. The cost function is the negative log-likelihood. The embedding for a word is obtained as the output of the embedding layer when the one-hot encoding of this word is given as the input to the model. 

Because there are so many parameters in the word2vec model, two techniques are used to make the computation more efficient: *hierarchical softmax* (faster way of computing softmax) and *negative sampling* (only updating a random sample of all outputs per iteration of gradient descent). 
...
In text analysis, topic modeling is a prevalent unsupervised learning problem. You have a collection of text documents and you want to discover topics present in each document. **Latent Dirichlet Allocation** is a popular algirithm of topic discovery. 

### Gaussian Processes

**Gaussian Processes** is a supervised learning method that competes with kernel regression. It has some advantages over the latter. It provides confidence intervals for the regression line at each point. 

### Generalized Linear Models

**Generalized Linear Model** is a generalization of the linear regression to modeling various forms of dependency between the input feature vector and the target. Logistic regression, for instance, is one form of GLMs. 

### Probabilistic Graphical Models

One example of **probabilistic graphical models** that was mentioned were the **conditional random fields** in chapter 7. With CRF you can model the input sequence of words and relationships between the features and labels in this sequence as a sequential dependency graph. A PGM can be any graph. A **graph** is a structure consisting of a collection of nodes and edges that each join a pair of nodes. Each node in PGM represents some random variable, and edges represent the conditional dependence of one random variable on another random variable. 

PGMs are also called Bayesian networks, and are quite popular.

### Markov Chain Monte Carlo

If you want to sample examples from very complex distributions defined by a dependency graph, you can use the **Markov Chain Monte Carlo** algorithms. MCMC is a class of algorithms for sampling from any probability distribution. The task of sampling becomes hard whent he probability distribution can have an arbitrary form defined by a complex formula.

### Generative Adversarial Networks

**Generative Adversarial Networks** (GANs) are a class of neural networks in unsupervised learning. The system consists of two neural networks contetsting with each other in a **zero-sum game**, where only one is the winner. The most popular use of GANs is to learn to generate photographs to learn authentic to human observers. The first network takes a rando input, and learns to generate an image as a matrix of pixels. The second networks takes as input two image: the real image, and the image generated by the first network, and has to differentiate between the two and pick out which one was generated by the model. The frst networks gets a negative loss if the second network recognizes the fake image. THe second network, gets penalized if it fails to recognize the fake image.

### Genetic Algorithms

**Genetic Algorithms** are a numerical optimization technique used to optimize undifferentiable optimization objective functions. They use concepts from evolutionary biology to search for a global optimum of an optimization problem using evolutionary ideas.

GA works by starting with an initial generation of candidate solutions. To find the optimal values of the parameters of our model, we first generate a set of random parameter values. Testing each combination of values against the objective function, we can find one/some that are better than the others. We then generate a subsequent generation of points from the previous generation by using "selection", "crossover", and "mutation".

This results in each new generation keeping more similar points to the points from the previous generation that performed the best. The points that performed the worst are replaced by "mutations" and "crossovers". A mutation of a point is a random distortion of some attributes of the original point. A crossover is a vertain combination of several points.

This is usually much slower than gradient-based optimization techniques.

### Reinforcement Learning

Gotta talk about **reinforcement learning** (RL), which solves a specific kind of problem where the decision maiking is sequential. There is an agent, acting in an unknown environment. Each action brings a reward and moves the agent into another state. The goal of the agent is to optimize the long-term reward.

Reinforcement learning algorithms such as Q-learning and their neural network based counterparts are used in learning to play video games, robotic navagation, etc.
